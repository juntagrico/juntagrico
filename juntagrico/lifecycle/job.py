...
def job_pre_save(sender, instance, **kwargs):
    ...
    if instance._old['time'] is not None and instance._old['time'] != instance.time:
        job_time_changed.send(sender=sender, instance=instance)


def handle_job_canceled(sender, instance, **kwargs):
    if instance.pk:
        instance.assignment_set.update(amount=0)
        membernotification.job_canceled(instance)


def handle_job_time_changed(sender, instance, **kwargs):
    membernotification.job_time_changed(instance)
...
def check_job_consistency(instance):
    ...
    if not instance.canceled and not instance.infinite_slots and instance.slots <= 0:
