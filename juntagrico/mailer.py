...
class BillingEmails:
    @staticmethod
    def share(bill, share, member):
        send_mail(
            organisation_subject(_('Rechnung {}').format(Config.vocabulary('share'))),
            get_email_content('b_share', base_dict(locals())),
            member.email
        )

    @staticmethod
    def subscription(bill, sub, start, end, member):
        send_mail(
            organisation_subject(_('Rechnung {}').format(Config.vocabulary('subscription'))),
            get_email_content('b_sub', base_dict(locals())),
            member.email
        )

    @staticmethod
    def extrasub(bill, extrasub, start, end, member):
        send_mail(
