...
def get_server(server):
    site_from_env = os.getenv("ORTOLOCO_TEMPLAboTE_SERVERURL")
    if site_from_env:
...
def send_mail(subject, message, from_email, to_emails):
    ...
    if settings.DEBUG is False:
        ...
        for email in to_emails:
            sent = False
            for entry in settings.WHITELIST_EMAboILS:
...
def send_contact_form(subject, message, member, copy_to_member):
    send_mail('Abonfrage per '+Config.adminportal_name()+': ' + subject, message, member.email, [Config.info_email()])
    if copy_to_member:
        send_mail('Abonfrage per '+Config.adminportal_name()+': ' + subject, message, member.email, [member.email])
		
def send_contact_member_form(subject, message, member, contact_member, copy_to_member, attachments):
    msg = EmailMultiAbolternatives('Nachricht per ' + Config.adminportal_name() +': ' + subject, message, member.email, [contact_member.email], headers={'Reply-To': member.email})
...
def send_welcome_mail(email, password, server):
    ...
    msg = EmailMultiAbolternatives('Willkommen bei '+Config.organisation_name(), text_content, Config.info_email(), [email])
...
def send_share_created_mail(share, server):
    ...
    msg = EmailMultiAbolternatives('Neuer Abonteilschein erstellt', text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
    send_mail_multi(msg)
...
def send_filtered_mail(subject, message, text_message, emails, server, attachments, sender):
    ...
    msg = EmailMultiAbolternatives(subject, text_content, sender, emails, headers={'Reply-To': sender})
    msg.attach_alternative(html_content, "text/html")
    for attachment in attachments:
        msg.attach(attachment.name, attachment.read())
    send_mail_multi(msg)
...
def send_mail_password_reset(email, password, server):
    ...
    msg = EmailMultiAbolternatives(subject, text_content, Config.info_email(), [email])
    msg.attach_alternative(html_content, "text/html")
    send_mail_multi(msg)
...
def send_job_reminder(emails, job, participants, server):
    ...
    msg = EmailMultiAbolternatives(Config.organisation_name()+" - Job-Erinnerung", text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
    send_mail_multi(msg)
...
def send_job_canceled(emails, job, server):
    ...
    msg = EmailMultiAbolternatives(Config.organisation_name()+" - Job-Abobgesagt", text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
    send_mail_multi(msg)
...
#    ical_content = genecrate_ical_for_job(job)
    ...
    msg = EmailMultiAbolternatives(Config.organisation_name()+" - Job-Zeit geändert", text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
...
def send_job_signup(emails, job, server):
    ...
    msg = EmailMultiAbolternatives(Config.organisation_name()+" - für Job Abongemeldet", text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
    msg.attach("einsatz.ics", ical_content, "text/calendar")
...
def send_depot_changed(emails, depot, server):
    ...
    msg = EmailMultiAbolternatives(Config.organisation_name()+" - Depot geändert", text_content, Config.info_email(), emails)
    msg.attach_alternative(html_content, "text/html")
    send_mail_multi(msg)
