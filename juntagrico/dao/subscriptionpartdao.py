...
from juntagrico.util.models import q_activated, q_canceled, q_isactive, q_deactivated
...
class SubscriptionPartDao:
    ...
    def get_canceled_for_subscription(subscription):
        return SubscriptionPart.objects.filter(subscription=subscription).filter(q_canceled())
    ...
    def future_extrasubscriptions():
        return SubscriptionPart.objects.filter(type__size__product__is_extra=True)\
            .filter(~q_canceled() & ~q_deactivated()).filter(deactivation_date=None)
    ...
    def canceled_extra_subs():
        return SubscriptionPart.objects.filter(type__size__product__is_extra=True).filter(q_canceled() & ~q_deactivated())
    ...
    def canceled_parts_for_active_subscriptions():
        return SubscriptionPart.objects.filter(type__size__product__is_extra=False).filter(q_canceled() & ~q_deactivated()).filter(
