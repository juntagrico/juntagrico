...
class JuntagricoUserAdmin(UserAdmin):
    readonly_fields = ('member_link',)
    list_display = ('username', 'member', 'is_staff')

    def __init__(self, model, admin_site):
        super().__init__(model, admin_site)
        self.fieldsets = (self.fieldsets[0], *self.fieldsets[2:])  # remove irrelevant section
        self.fieldsets[0][1]['fields'] += ('member_link',)  # add link to member

    def member_link(self, obj):
        return mark_safe('<a href="{}">{} ({})</a>'.format(
            reverse("admin:juntagrico_member_change", args=(obj.member.pk,)),
            obj.member, obj.member.email
        ))
    member_link.short_description = Config.vocabulary('member')
