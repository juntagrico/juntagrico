...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_column(u'loco_app_job', 'start_time')

        # Adding field 'Job.time'
        db.add_column(u'loco_app_job', 'time',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime(2013, 7, 17, 0, 0)),
                      keep_default=False)

        # Adding unique constraint on 'Depot', fields ['name']
        db.create_unique(u'loco_app_depot', ['name'])

        # Adding unique constraint on 'ExtraAboType', fields ['name']
        db.create_unique(u'loco_app_extraabotype', ['name'])

        # Adding unique constraint on 'JobTyp', fields ['name']
        db.create_unique(u'loco_app_jobtyp', ['name'])

        # Adding unique constraint on 'Taetigkeitsbereich', fields ['name']
        db.create_unique(u'loco_app_taetigkeitsbereich', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Taetigkeitsbereich', fields ['name']
        db.delete_unique(u'loco_app_taetigkeitsbereich', ['name'])

        # Removing unique constraint on 'JobTyp', fields ['name']
        db.delete_unique(u'loco_app_jobtyp', ['name'])

        # Removing unique constraint on 'ExtraAboType', fields ['name']
        db.delete_unique(u'loco_app_extraabotype', ['name'])

        # Removing unique constraint on 'Depot', fields ['name']
        db.delete_unique(u'loco_app_depot', ['name'])

        # Adding field 'Job.start_time'
        db.add_column(u'loco_app_job', 'start_time',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime(2013, 7, 17, 0, 0)),
                      keep_default=False)

        # Deleting field 'Job.time'
        db.delete_column(u'loco_app_job', 'time')
