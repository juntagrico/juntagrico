...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for job in orm.Job.objects.all():
            newJob = orm.RecuringJob()
            newJob.slots = job.slots
            newJob.time = job.time
            newJob.pinned = job.pinned
            newJob.reminder_sent = job.reminder_sent
            newJob.canceled = job.canceled
            newJob.typ = job.typ
            newJob.jobType = job.typ
            newJob.save()
            for boehnli in orm.Boehnli.objects.filter(job_id=job.id):
                boehnli.job=newJob
                boehnli.save()
            job.delete()

    def backwards(self, orm):
        for job in orm.RecuringJob.objects.all():
            newJob = orm.Job()
            newJob.slots = job.slots
            newJob.time = job.time
            newJob.pinned = job.pinned
